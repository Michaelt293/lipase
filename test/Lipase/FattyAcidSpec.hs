...
spec = do
  describe "Elemental composition for FattyAcyl" $ do
    it "Stearic acid acyl group elemental composition should be C18H35O2" $
      toMolecularFormula (FattyAcyl 18 0)
      `shouldBe` mkMolecularFormula [(C, 18), (H, 35), (O, 2)]
    it "Oleic acid acyl group elemental composition should be C18H33O2" $
      toMolecularFormula (FattyAcyl 18 1)
      `shouldBe` mkMolecularFormula [(C, 18),(H, 33), (O, 2)]
    it "DHA elemental acyl group composition should be C22H31O2" $
      toMolecularFormula (FattyAcyl 22 6)
      `shouldBe` mkMolecularFormula [(C, 22),(H, 31), (O, 2)]

  describe "Elemental composition for FattyAcid" $ do
    it "Stearic acid elemental composition should be C18H36O2" $
      toMolecularFormula (FattyAcid (FattyAcyl 18 0))
      `shouldBe` mkMolecularFormula [(C, 18), (H, 36), (O, 2)]
    it "Oleic acid elemental composition should be C18H34O2" $
      toMolecularFormula (FattyAcid (FattyAcyl 18 1))
      `shouldBe` mkMolecularFormula [(C, 18),(H, 34), (O, 2)]
    it "DHA elemental composition should be C22H32O2" $
      toMolecularFormula (FattyAcid (FattyAcyl 22 6))
      `shouldBe` mkMolecularFormula [(C, 22),(H, 32), (O, 2)]
